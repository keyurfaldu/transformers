...
class BertForNLI(BertPreTrainedModel):
    def __init__(self, config, sequence_length=256):
        super().__init__(config)
        self.num_labels = config.num_labels

        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        #self.dense_projection = nn.Linear(config.hidden_size*sequence_length, config.hidden_size)
    ...
    def forward(
        ...
        sequence_hidden_states = outputs[0]
        pooled_output = outputs[1]

        #sequence_output = self.dense_projection(sequence_hidden_states.flatten(start_dim=1))
        #sequence_output = self.dropout(sequence_output)
        #mean_sequence_output = sequence_hidden_states.mean(dim=1)
...
class BertForSpanClassification(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = nn.Linear(self.num_spans*config.hidden_size, config.num_labels)
